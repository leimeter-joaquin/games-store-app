...
const props = withDefaults(
  defineProps<{
    ...
    products: Product[]; // Change to "items"
  }>(),
  {
    containerHeight: 300,
    containerWidth: 300,
...
function prev() {
  ...
  currentIndex.value =
    (currentIndex.value + products.length - 1) % products.length;
...
<template>
  <div class="relative inline-flex flex-col items-center">
    ...
    <div
      class="absolute bottom-[3%] inline-flex items-baseline gap-2 rounded bg-white bg-opacity-20 py-1 px-2"
    >
      <Icon
        name="ic:baseline-circle"
        v-for="(product, index) in products"
        class="cursor-pointer transition duration-1000 ease-out"
